...
import com.hmwl.mapper.GoodsImageMapper;
...
public class GoodsImageServiceImpl extends ServiceImpl<GoodsImageMapper, GoodsImage> implements GoodsImageService {
    ...
    @Override
    public GoodsImage uploadToCos(Long orderId, String orderNo, MultipartFile file) throws Exception {
        String originalFilename = file.getOriginalFilename();
        String suffix = originalFilename.substring(originalFilename.lastIndexOf("."));
        String timestamp = String.valueOf(System.currentTimeMillis());
        String filename = "goods/" + orderNo + "/" + "goods_" + orderNo + "_" + timestamp + suffix;

        try {
            System.out.println("开始上传货物图片到COS，bucketName: " + bucketName);
            ...
            goodsImage.setOrderId(orderId);
            goodsImage.setImageUrl(imageUrl);
            goodsImage.setStatus(1);
            goodsImage.setCreateTime(new java.util.Date());
            goodsImage.setUpdateTime(new java.util.Date());
            save(goodsImage);
    ...
    @Override
    public List<GoodsImage> listByOrderId(Long orderId) {
        List<GoodsImage> list = baseMapper.selectList(new com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper<GoodsImage>()
                .eq(GoodsImage::getOrderId, orderId)
                .eq(GoodsImage::getStatus, 1));
        
        for (GoodsImage image : list) {
    ...
    public boolean deleteImage(Long id) throws Exception {
        GoodsImage goodsImage = baseMapper.selectById(id);
        if (goodsImage == null) {
        ...
        if (imageUrl != null && !imageUrl.isEmpty() && imageUrl.startsWith(baseUrl)) {
            String cosKey = imageUrl.substring(baseUrl.length() + 1);
            if (cosKey.contains("?")) {
                cosKey = cosKey.substring(0, cosKey.indexOf("?"));
            }
